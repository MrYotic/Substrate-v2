...
/// <summary> Provides read-only indexed access to an underlying resource. </summary>
...
/*internal class CacheTableArray<T> : ICacheTable<T>
{
    ...
    public CacheTableArray (T[] cache)= > _cache = cache;
}
internal class CacheTableDictionary<T> : ICacheTable<T>
{
    ...
    public CacheTableDictionary (Dictionary<int, T> cache) => _cache = cache;
}
...
public class CacheTable<T>
{
    ...
    internal CacheTable (T[] cache) => _cache = new CacheTableArray<T>(cache);
    internal CacheTable (Dictionary<int, T> cache) => _cache = new CacheTableDictionary<T>(cache);
}*/
