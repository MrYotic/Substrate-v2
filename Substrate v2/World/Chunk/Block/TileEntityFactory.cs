...
/// <summary> Creates new instances of concrete <see cref="TileEntity"/> types from a dynamic registry. </summary>
...
public class TileEntityFactory
{
    ...
    /// <summary> Create a new instance of a concrete <see cref="TileEntity"/> type by name. </summary>
    ...
    /// <summary> Create a new instance of a concrete <see cref="TileEntity"/> type by NBT node. </summary>
    ...
    {
        Type t;
        if (!_registry.TryGetValue(tree["id"].ToTagString(), out t))
            return null;
        return (Activator.CreateInstance(t) as TileEntity).LoadTreeSafe(tree);
    }
    /// <summary> Create a new instance of a concrete <see cref="TileEntity"/> type by NBT node. </summary>
    /// <param name="tree">A <see cref="TagNodeCompound"/> representing a single Tile Entity, containing an 'id' field of the Tile Entity's registered name.</param>
    /// <returns>A new instance of a concrete <see cref="TileEntity"/> type, or null if no type was registered with the given name.</returns>
    public static TileEntity CreateGeneric(TagNodeCompound tree)
    {
        ...
        if (!_registry.TryGetValue(tree["id"].ToTagString(), out t))
            t = typeof (TileEntity);          
        return (Activator.CreateInstance(t, true) as TileEntity).LoadTreeSafe(tree);
    }
    /// <summary> Lookup a concrete <see cref="TileEntity"/> type by name. </summary>
    /// <param name="type">The name that a concrete <see cref="TileEntity"/> type was registered with.</param>
    ...
    /// <summary> Registers a new concrete <see cref="TileEntity"/> type with the <see cref="TileEntityFactory"/>, binding it to a given name. </summary>
    ...
    public static void Register (string id, Type subtype) => _registry[id] = subtype;
    /// <summary> Gets an enumerator over all registered TileEntities. </summary>
...
/// <summary> An exception that is thrown when unknown TileEntity types are queried. </summary>
