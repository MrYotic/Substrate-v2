...
/// <summary> Represents an Alpha-compatible interface for globally managing blocks. </summary>
public abstract class BlockManager : IVersion10BlockManager, IBlockManager
{
    ...
    /// <summary> Gets or sets a value indicating whether changes to blocks will trigger automatic lighting updates. </summary>
    public bool AutoLight { get; set; }
    /// <summary> Gets or sets a value indicating whether changes to blocks will trigger automatic fluid updates. </summary>
    public bool AutoFluid { get; set; }
    /// <summary> Gets or sets a value indicating whether changes to blocks will trigger automatic fluid updates. </summary>
    public bool AutoTileTick { get; set; }   
    /// <summary> Constructs a new <see cref="BlockManager"/> instance on top of the given <see cref="IChunkManager"/>. </summary>
    /// <param name="cm">An <see cref="IChunkManager"/> instance.</param>
    public BlockManager (IChunkManager cm) => chunkMan = cm;

    /// <summary> Returns a new <see cref="AlphaBlock"/> object from global coordinates. </summary>
    ...
    /// <summary> Returns a new <see cref="AlphaBlockRef"/> object from global coordaintes. </summary>
    /// <param name="x">Global X-coordinate of block.</param>
    /// <param name="y">Global Y-coordinate of block.</param>
    ...
    /// <summary> Updates a block with values from a <see cref="AlphaBlock"/> object. </summary>
    ...
    /// <summary> Gets a reference object to a single chunk given global coordinates to a block within that chunk. </summary>
    /// <param name="x">Global X-coordinate of a block.</param>
    /// <param name="y">Global Y-coordinate of a block.</param>
    /// <param name="z">Global Z-coordinate of a block.</param>
    ...
    /// <summary> Called by other block-specific 'get' and 'set' functions to filter
    /// out operations on some blocks.  Override this method in derrived
    /// classes to filter the entire BlockManager. </summary>
    protected virtual bool Check (int x, int y, int z) => (x >= MIN_X) && (x < MAX_X) && (y >= MIN_Y) && (y < MAX_Y) && (z >= MIN_Z) && (z < MAX_Z);
    IBlock IBlockCollection.GetBlock (int x, int y, int z) => GetBlock(x, y, z);
    IBlock IBlockCollection.GetBlockRef (int x, int y, int z) => GetBlockRef(x, y, z);
    ...
    IDataBlock IDataBlockCollection.GetBlock (int x, int y, int z) => GetBlock(x, y, z);
    IDataBlock IDataBlockCollection.GetBlockRef (int x, int y, int z) => GetBlockRef(x, y, z);
    ...
    ILitBlock ILitBlockCollection.GetBlock (int x, int y, int z) => throw new NotImplementedException();

    ILitBlock ILitBlockCollection.GetBlockRef (int x, int y, int z) => GetBlockRef(x, y, z);
    ...
    IPropertyBlock IPropertyBlockCollection.GetBlock (int x, int y, int z) => GetBlock(x, y, z);
    IPropertyBlock IPropertyBlockCollection.GetBlockRef (int x, int y, int z) => GetBlockRef(x, y, z);
    ...
    IActiveBlock IActiveBlockCollection.GetBlock (int x, int y, int z) => GetBlock(x, y, z);
    IActiveBlock IActiveBlockCollection.GetBlockRef (int x, int y, int z) => GetBlockRef(x, y, z);
