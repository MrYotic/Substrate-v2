...
/// <summary> Provides a wrapper around a physical Chunk stored in a chunk container. </summary>
/// <remarks> Modifying data in a ChunkRef will signal to the chunk container that the physical chunk needs to be saved. </remarks>
public class ChunkRef : IChunk
{
    ...
    /// <summary> Gets the global X-coordinate of the chunk. </summary>
    public int X
    {
        get => container.ChunkGlobalX(cx);
    }
    /// <summary> Gets the global Z-coordinate of the chunk. </summary>
    public int Z
    {
        get => container.ChunkGlobalZ(cz);
    }
    /// <summary> Gets the local X-coordinate of the chunk within container. </summary>
    public int LocalX
    {
        get => container.ChunkLocalX(cx);
    }
    /// <summary> Gets the local Z-coordinate of the chunk within container. </summary>
    public int LocalZ
    {
        get => container.ChunkLocalZ(cz);
    }
    /// <summary> Gets the collection of all blocks and their data stored in the chunk. </summary>
    ...
    /// <summary> Gets the collection of all entities stored in the chunk. </summary>
    ...
    /// <summary> Gets or sets the value indicating that the chunk has been modified, but not saved. </summary>
    public bool IsDirty
    {
        get => dirty || (blocks != null && blocks.IsDirty) || (entities != null && entities.IsDirty);
        set
        {
            dirty = value;
            if (blocks != null)
                blocks.IsDirty = false;
            if (entities != null)
                entities.IsDirty = false;
        }
    }
    /// <summary> Forbid direct instantiation of ChunkRef objects </summary>
    private ChunkRef () { }
    /// <summary> Create a reference to a chunk stored in a chunk container. </summary>
    ...
    /// <summary> Gets or sets the chunk's TerrainPopulated status. </summary>
    public bool IsTerrainPopulated
    {
        get => GetChunk().IsTerrainPopulated;
        set
        {
    ...
    /// <summary> Saves the underlying physical chunk to the specified output stream. </summary>
    ...
    public void SetLocation (int x, int z)
    {
        int relX = LocalX + (x - X);
        int relZ = LocalZ + (z - Z);
        ChunkRef c = container.SetChunk(relX, relZ, GetChunk());
        container = c.container;
        cx = c.cx;
        cz = c.cz;
    }
    /// <summary> Gets a ChunkRef to the chunk positioned immediately north (X - 1). </summary>
    /// <returns>ChunkRef to the northern neighboring chunk.</returns>
    public ChunkRef GetNorthNeighbor () => container.GetChunkRef(cx - 1, cz);  
    /// <summary> Gets a ChunkRef to the chunk positioned immediately south (X + 1). </summary>
    /// <returns>ChunkRef to the southern neighboring chunk.</returns>
    public ChunkRef GetSouthNeighbor() => container.GetChunkRef(cx + 1, cz);

    /// <summary> Gets a ChunkRef to the chunk positioned immediatly east (Z - 1). </summary>
    /// <returns>ChunkRef to the eastern neighboring chunk.</returns>
    public ChunkRef GetEastNeighbor () => container.GetChunkRef(cx, cz - 1);
    /// <summary> Gets a ChunkRef to the chunk positioned immedately west (Z + 1). </summary>
    /// <returns>ChunkRef to the western neighboring chunk.</returns>
    public ChunkRef GetWestNeighbor () => container.GetChunkRef(cx, cz + 1);

    /// <summary> Returns a deep copy of the physical chunk underlying the ChunkRef. </summary>
    ...
    /// <summary> Returns the reference of the physical chunk underlying the ChunkRef, and releases the reference from itself. </summary>
    /// <remarks>
    ...
    /// <summary> Replaces the underlying physical chunk with a different one, updating its physical location to reflect the ChunkRef. </summary>
    /// <remarks>
    ...
    /// <summary> Gets an internal Chunk reference from cache or queries the container for it. </summary>
    ...
    /// <summary> Callback function to return the block collection of a ChunkRef at a relative offset to this one. </summary>
    ...
    /// <summary> Translates chunk-local block coordinates to corresponding global coordinates. </summary>
