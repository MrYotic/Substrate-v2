...
/// <summary> Functions to manage multiple <see cref="Player"/> entities and files in multiplayer settings. </summary>
...
public class PlayerManager : IPlayerManager, IEnumerable<Player>
{
    ...
    /// <summary> Create a new <see cref="PlayerManager"/> for a given file path. </summary>
    /// <param name="playerDir">Path to a directory containing player data files.</param>
    public PlayerManager (string playerDir) => playerPath = playerDir;
    /// <summary> Gets a <see cref="PlayerFile"/> representing the backing NBT data stream. </summary>
    /// <param name="name">The name of the player to fetch.</param>
    /// <returns>A <see cref="PlayerFile"/> for the given player.</returns>
    protected PlayerFile GetPlayerFile (string name) => new PlayerFile(playerPath, name);
    /// <summary> Gets a raw <see cref="NbtTree"/> of data for the given player. </summary>
    /// <param name="name">The name of the player to fetch.</param>
    ...
    /// <summary> Saves a raw <see cref="NbtTree"/> representing a player to the given player's file. </summary>
    ...
    /// <summary> Gets a <see cref="Player"/> object for the given player. </summary>
    /// <param name="name">The name of the player to fetch.</param>
    ...
    /// <summary> Saves a <see cref="Player"/> object's data back to the given player's file. </summary>
    ...
    /// <summary> Saves a <see cref="Player"/> object's data back to file given the name set in the <see cref="Player"/> object. </summary>
    /// <param name="player">The <see cref="Player"/> object containing the data to write back.</param>
    /// <exception cref="PlayerIOException">Thrown when the manager cannot write out the player.</exception>
    public void SetPlayer (Player player) => SetPlayer(player.Name, player);
    /// <summary> Checks if data for a player with the given name exists. </summary>
    /// <param name="name">The name of the player to look up.</param>
    /// <returns>True if player data was found; false otherwise.</returns>
    public bool PlayerExists (string name) => new PlayerFile(playerPath, name).Exists();

    /// <summary> Deletes a player with the given name from the underlying data store. </summary>
    ...
    {
        try {
            new PlayerFile(playerPath, name).Delete();
        }
        catch (Exception ex) {
    ...
    /// <summary> Gets an enumerator that iterates through all the players in the world. </summary>
    /// <returns>An enumerator for this manager.</returns>
    public IEnumerator<Player> GetEnumerator() => new Enumerator(this);
    ...
    private class Enumerator : IEnumerator<Player>
    {
        ...
        public Enumerator (PlayerManager cfm)
        {
            ...
            if (!Directory.Exists(_pm.playerPath))
        ...
        public void Reset ()
        {
            _names.Clear();
            _curPlayer = null;
            string[] files = Directory.GetFiles(_pm.playerPath);
        ...
        Player IEnumerator<Player>.Current { get => Current; }
